...
private:
	...
	template <typename T> void insert(T data, unsigned int index) {
		...
		memcpy(&buf[index], (byteT*)&data, sizeof(data));
		wpos = index+sizeof(data);
	}

public:
	ByteBuffer(unsigned int size = DEFAULT_SIZE);
	ByteBuffer(byteT* arr, unsigned int size);
	~ByteBuffer();
    ...
    // Replacement
    void replace(byteT key, byteT rep, unsigned int start = 0, bool firstOccuranceOnly=false);
	
	// Read

	byteT peek(); // Relative peek. Reads and returns the next byte in the buffer from the current position but does not increment the read position
	byteT get(); // Relative get method. Reads the byte at the buffers current position then increments the position
	byteT get(unsigned int index); // Absolute get method. Read byte at index
	void getBytes(byteT* buf, unsigned int len); // Absolute read into array buf of length len
	...
	void put(byteT b); // Relative write
	void put(byteT b, unsigned int index); // Absolute write at index
	void putBytes(byteT* b, unsigned int len); // Relative write
	void putBytes(byteT* b, unsigned int len, unsigned int index); // Absolute write starting at index
