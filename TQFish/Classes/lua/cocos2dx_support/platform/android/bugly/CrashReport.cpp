...
void CrashReport::initCrashReport(const char* appId, bool isDebug) {
    CrashReport::initCrashReport(appId, isDebug, 3);
}

void CrashReport::initCrashReport(const char* appId, bool isDebug, int level) {
	if (!initialized) {
...
void CrashReport::log(int level, const char * tag, const char * fmts, ...) {
#if (CC_TARGET_PLATFORM == CC_PLATFORM_ANDROID)
    JavaVM* jvm = cocos2d::JniHelper::getJavaVM();
	if (jvm == NULL) {
		LOGE("[cocos2d-x] JavaVM is null.");
