...
bool CCLuaJavaBridge::CallInfo::execute(void) {
	...
	case TypeString:
		m_retjs = (jstring) m_env->CallStaticObjectMethod(m_classID,
				m_methodID);
...
bool CCLuaJavaBridge::CallInfo::executeWithArgs(jvalue *args) {
	...
	case TypeInteger:
		m_ret.intValue = m_env->CallStaticIntMethodA(m_classID, m_methodID,
				args);
		break;

	case TypeFloat:
		m_ret.floatValue = m_env->CallStaticFloatMethodA(m_classID, m_methodID,
				args);
		break;

	case TypeBoolean:
		m_ret.boolValue = m_env->CallStaticBooleanMethodA(m_classID, m_methodID,
				args);
		break;

	case TypeString:
		m_retjs = (jstring) m_env->CallStaticObjectMethodA(m_classID,
				m_methodID, args);
		const char *stringBuff = m_env->GetStringUTFChars(m_retjs, 0);
		m_ret.stringValue = new string(stringBuff);
		m_env->ReleaseStringUTFChars(m_retjs, stringBuff);
...
CCLuaJavaBridge::ValueType CCLuaJavaBridge::CallInfo::checkType(
		const string& sig, size_t *pos) {
	switch (sig[*pos]) {
	case 'I':
		return TypeInteger;
	case 'F':
...
bool CCLuaJavaBridge::CallInfo::getMethodInfo(void) {
	...
	m_methodID = m_env->GetStaticMethodID(m_classID, m_methodName.c_str(),
			m_methodSig.c_str());
	if (!m_methodID) {
		m_env->ExceptionClear();
		LOGD("Failed to find method id of %s.%s %s", m_className.c_str(),
				m_methodName.c_str(), m_methodSig.c_str());
...
int CCLuaJavaBridge::callJavaStaticMethod(lua_State *L) {
	if (!lua_isstring(L, -4) || !lua_isstring(L, -3) || !lua_istable(L, -2)
			|| !lua_isstring(L, -1)) {
		lua_pushboolean(L, 0);
	...
	if (!call.isValid()) {
		LOGD(
				"CCLuaJavaBridge::callJavaStaticMethod(\"%s\", \"%s\", args, \"%s\") CHECK FAILURE, ERROR CODE: %d",
	...
	if (!success) {
		LOGD("CCLuaJavaBridge::callJavaStaticMethod(\"%s\", \"%s\", args, \"%s\") EXECUTE FAILURE, ERROR CODE: %d",className, methodName, methodSig, call.getErrorCode());
...
int CCLuaJavaBridge::retainLuaFunctionById(int functionId) {
	...
	LOGD("luajretainLuaFunctionById(%d) - retain count = %d", functionId,
			retainCount);
...
int CCLuaJavaBridge::releaseLuaFunctionById(int functionId) {
	...
	if (!lua_istable(L, -1)) {
		lua_pop(L, 1);
		LOGD("%s",
				"luajreleaseLuaFunctionById() - LUAJ_REGISTRY_FUNCTION not exists");
		return 0;
	}
	...
	if (!lua_istable(L, -1)) {
		lua_pop(L, 2);
		LOGD("%s",
				"luajreleaseLuaFunctionById() - LUAJ_REGISTRY_RETAIN not exists");
		return 0;
	}
	...
	if (lua_type(L, -1) != LUA_TNUMBER) {
		lua_pop(L, 3);
		LOGD("luajreleaseLuaFunctionById() - function id %d not found",
				functionId);
		return 0;
	}
	...
	if (retainCount > 0) {
		...
		lua_pop(L, 2);
		LOGD("luajreleaseLuaFunctionById() - function id %d retain count = %d",
				functionId, retainCount);
		return retainCount;
	}
...
int CCLuaJavaBridge::callLuaGlobalFunction(const char *functionName,
		const char *arg) {
	lua_State *L = s_luaState;
...
int CCLuaJavaBridge::retainLuaFunction(lua_State *L, int functionIndex,
