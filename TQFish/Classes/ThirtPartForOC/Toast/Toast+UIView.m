                     ...
                     animations:^{
                         ...
                         [UIView animateWithDuration:CSToastFadeDuration
                                          ...
                                          animations:^{
                                              ...
                                              //[[[[UIApplication sharedApplication] keyWindow] viewWithTag:30000] removeFromSuperview];
                                          }];
...
- (CGPoint)centerPointForPosition:(id)point withToast:(UIView *)toast {
    if([point isKindOfClass:[NSString class]]) {
        ...
        if([point caseInsensitiveCompare:@"top"] == NSOrderedSame) {
            ...
            return CGPointMake(self.bounds.size.height/2, (self.bounds.size.width - (toast.frame.size.width / 2)) - CSToastVerticalPadding);
        } else if([point caseInsensitiveCompare:@"center"] == NSOrderedSame) {
            //ios 7  和之前的系统height 和 width的返回值不一样，是反的。。。
            if (self.bounds.size.width < self.bounds.size.height)
            {
                return CGPointMake(self.bounds.size.height / 2, self.bounds.size.width / 2);
            }
            else
            {
