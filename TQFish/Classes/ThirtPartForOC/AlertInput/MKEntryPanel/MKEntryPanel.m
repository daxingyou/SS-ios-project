...
+(void) showPanelWithTitle:(NSString *)title content:(NSString *)msg inView:(UIView *)view onTextEntered:(CloseBlock)editingEndedBlock
{
    MKEntryPanel *panel = [MKEntryPanel panel];
    CGRect frame = panel.frame;
    frame.origin.x = (CGRectGetWidth(view.frame) - CGRectGetWidth(panel.frame))/2;
    panel.frame = frame;
    panel.closeBlock = editingEndedBlock;
    panel.titleLabel.text = title;
    panel.entryField.text = msg;
    ...
    panel.dimView.alpha = 0.1;
    [view addSubview:panel.dimView];
    panel.dimView.frame = ((UIView *)[EAGLView sharedEGLView]).bounds;
...
-(void) hidePanel
{
    ...
    [self.dimView performSelector:@selector(removeFromSuperview) withObject:nil afterDelay:0.001];
