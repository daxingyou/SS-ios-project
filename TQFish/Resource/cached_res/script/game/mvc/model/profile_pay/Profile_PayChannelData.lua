...
local rechargeRebateInfo = {}  --首充翻倍信息
...
function readPAYMENT_DATA_LIST(dataTable)
	...
	for key, var in ipairs(PayChannelTable["PayChannelData"]) do
		...
		if var.PayTypeID == ALI_PAY then
			...
			tempTable.addYuanbao = var.addYuanbao -- 额外赠送的元宝数
...
function readRECHARGE_REBATE_INFO(dataTable)
	rechargeRebateInfo = dataTable

	framework.emit(RECHARGE_REBATE_INFO)
end
...
function getRECHARGE_REBATE_INFO()
	return rechargeRebateInfo
end

--[[--
--是否是首充翻倍
]]
function IsRechargeRebate()
	--Byte	是否为首充翻倍	1:是 0不是
	local flag = rechargeRebateInfo["IsFirstRecharge"]
...
function getVipAddingPercent()
	local value = rechargeRebateInfo["VipAddingPercent"]
	if value ~= nil then
		return value
	else
		return 0
...
--获取支付列表数据
--]]
function getAllPayDataList()
	return PayChannelTable
end

registerMessage(PAYMENT_DATA_LIST,readPAYMENT_DATA_LIST)
registerMessage(RECHARGE_REBATE_INFO,readRECHARGE_REBATE_INFO)