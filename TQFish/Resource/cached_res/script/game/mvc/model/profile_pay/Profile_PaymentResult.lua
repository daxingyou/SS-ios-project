...
local PayOrderIdTable = {}
--iap支付成功后 苹果服务器通知客户端
--标记客户端是否在咱们自己的服务器去验证过
local iapOrderIdVerifyMarkTable = {}
--[[--
--获取iap支付的orderID
--]]
local function getPayOrderIdByProductID(ProductID)
	if PayOrderIdTable[ProductID] ~= nil and #PayOrderIdTable[ProductID] > 0 then
		local OrderId = PayOrderIdTable[ProductID][1];
		table.remove(PayOrderIdTable[ProductID], 1);
		Common.log("获取iap支付的orderID ====== " ..OrderId)
		return OrderId;
	end
	return nil;
end
...
--存储iap支付的orderID
--]]
local function addPayOrderId(ProductID, OrderId)
	if PayOrderIdTable[ProductID] == nil then
		PayOrderIdTable[ProductID] = {}
	end
	table.insert(PayOrderIdTable[ProductID], OrderId);
end


--[[--
--存储当前支付订单是否完成本地服务器验证
--OrderId 订单号  用来标记的key
--]]
function setIapOrderIdVerifyMark(OrderId,receipt)
	-- Common.log("OrderId =========================== "..OrderId)
	-- Common.log("receipt =========================== "..receipt)

	if iapOrderIdVerifyMarkTable == nil then
		iapOrderIdVerifyMarkTable = {}
	end
	if iapOrderIdVerifyMarkTable[OrderId] == nil then
		iapOrderIdVerifyMarkTable[OrderId] = {}
	end
	iapOrderIdVerifyMarkTable[OrderId].receipt = receipt
	--将 iapOrderIdVerifyMarkTable 存为json
	Common.SaveTable("iapOrderIdTable", iapOrderIdVerifyMarkTable)
end
function removeIapOrderIdVerifyMark(OrderId)
	-- Common.log("OrderId ===========================removeIapOrderIdVerifyMark "..OrderId)
	if iapOrderIdVerifyMarkTable == nil then
		iapOrderIdVerifyMarkTable = {}
	end
	iapOrderIdVerifyMarkTable[OrderId] = nil

	Common.SaveTable("iapOrderIdTable", iapOrderIdVerifyMarkTable)
end

--[[--
--断线重连回来 需要获取未验证的订单号 重新去验证
--]]
function verifyIapOrderId()
	if not GameChannelConfig.isAppStoreChannelID() then
		--如果不是ipple 渠道  不去检测
		return
	end
	--读取 iapOrderIdVerifyMarkTable 的Json数据
	iapOrderIdVerifyMarkTable = Common.LoadTable("iapOrderIdTable")
	-- Common.log("断线重连回来 需要获取未验证的订单号 重新去验证  ")
	if iapOrderIdVerifyMarkTable == nil then
		return
	end
	for orderId,value in pairs(iapOrderIdVerifyMarkTable) do
		-- Common.log("断线重连回来 需要获取未验证的订单号 重新去验证 orderId ============ "..orderId)
		local userid =  profile.User.getSelfUserID()
		sendMANAGERID_VALIDATE_IAP_V2(value.receipt,orderId,userid) --appstore审核版本
	end
end
...
function readMANAGERID_V3_RECHARGE(dataTable)
	...
	if (PayResultTable["result"] == 0) then
		if PayResultTable.payChannel == 0 and PayResultTable.Position ~= 0 and PayGuideDataTable ~= nil then
			...
			GiftGuideConfig.showPayGuide(tempTable.GuideType, tempTable.needCurrencyNum, tempTable.position, 0, PayResultTable["payMsg"])
		else
			--支付失败
		...
		if PayResultTable.SmsHint ~= nil and PayResultTable.SmsHint~= "" then
			PaymentConfig.showRechargePrompt(PayResultTable.SmsHint)
		end

		if PayResultTable["payChannel"] ==  profilePay.PayChannelData.RECHARGE_CARD_PAY then
		-- 充值卡子协议
		elseif PayResultTable["payChannel"]  ==  profilePay.PayChannelData.ALI_PAY then
			-- 支付宝子协议
			Common.setDataForSqlite(CommSqliteConfig.RECENT_RECHARGE_MEHTOD, "alipay");
			if Common.platform == Common.TargetIos then
				--ios平台
                CommDialogConfig.showProgressDialog();
                local alipayUrl = PayResultTable["KvLoop"].WAP_PAY_REQ_URL;
                Common.log("alipayUrl ======= "..alipayUrl);
                if alipayUrl ~= nil then
                    ...
                    luaoc.callStaticMethod("Helper", "openWebview", args);
                end
...
--				local ok, ret = luaoc.callStaticMethod("Helper", "alipayPaymentInfo", args);
			elseif Common.platform == Common.TargetAndroid then
				...
				function callAliPayGetOrderInfo(parameters)
					...
					local javaParams = {
						...
						PaymentConfig.showRechargePrompt,
					}
		...
		elseif PayResultTable["payChannel"] == profilePay.PayChannelData.WEIXIN_PAY then
			-- 微信子协议
			Common.setDataForSqlite(CommSqliteConfig.RECENT_RECHARGE_MEHTOD, "weixin");
			if Common.platform == Common.TargetIos then
				--ios平台
				local function installedCallBack(parameters)
					if parameters == nil or parameters == "" then
						return
					end

					if parameters == "HAVE_NO_WX" then
						Common.showToast("安装微信才可以购买哟 O(∩_∩)O",2)
					end
				end
				local args = {
					...
					installedCallBack = installedCallBack,
				}
				local ok, ret = luaoc.callStaticMethod("WechatConfig", "weixinPayResult", args);
			elseif Common.platform == Common.TargetAndroid then
				--Android平台
				local javaClassName = "org.tongqu.client.pay.PaymentConfig"
		...
		elseif PayResultTable["payChannel"]  ==  profilePay.PayChannelData.UNION_PAY then
			-- 银联子协议
			local javaClassName = "org.tongqu.client.pay.PaymentConfig"
		...
		elseif PayResultTable["payChannel"]  ==  profilePay.PayChannelData.SMS_UNICOM then
			-- 联通沃商店子协议
			if Common.platform == Common.TargetIos then
			--ios平台
			--				local args = {
			--					orderID = PayResultTable["OrderId"],
			--					callback = PaymentConfig.showRechargePrompt,
			--				};
		...
		elseif PayResultTable["payChannel"]  ==  profilePay.PayChannelData.MM_PAY_V2 then
			-- MM带回调
			local javaClassName = "org.tongqu.client.pay.PaymentConfig"
			...
			luaj.callStaticMethodPro(javaClassName, javaMethodName, javaParams, "V")
			--		elseif PayResultTable["payChannel"]  ==  profilePay.PayChannelData.CTESTORE_PAY then
			--			-- 天翼空间
			--			local javaClassName = "org.tongqu.client.pay.PaymentConfig"
			...
			--			}
			--			luaj.callStaticMethodPro(javaClassName, javaMethodName, javaParams, "V")
		elseif PayResultTable["payChannel"]  ==  profilePay.PayChannelData.RECHARGE_WOJIA then
			-- 同趣联通wo+
		...
		elseif PayResultTable["payChannel"] == profilePay.PayChannelData.NEW_UNION_PAY then
			...
			luaj.callStaticMethodPro(javaClassName, javaMethodName, javaParams, "V")
		elseif PayResultTable["payChannel"]  ==  profilePay.PayChannelData.RECHARGE_91 then
			-- 91
			--调用ios支付
		...
		elseif PayResultTable["payChannel"]  ==  profilePay.PayChannelData.HAIMA_PAY then
			-- 海马
			--调用ios支付
		...
		elseif PayResultTable["payChannel"]  ==  profilePay.PayChannelData.IAP_PAY then
			-- IAP支付
			local userid =  profile.User.getSelfUserID()
			local product_id = PayResultTable.KvLoop["product_id"];

			addPayOrderId(product_id, PayResultTable["OrderId"]);
			Common.log("传给iap支付的orderID ====== " ..PayResultTable["OrderId"])

			--iap
			function callIAPPayCheckInfo(parameters)
				local productid = parameters["productid"];
				local orderID = getPayOrderIdByProductID(productid)
				local receipt = parameters["receipt"];
				Common.log("IAP_PAY支付成功，告诉服务器去验证订单号 ===== 这里标记下")
				if orderID == nil then
					Common.showToast("订单号为空,请重试...",3)
					return
				end
				setIapOrderIdVerifyMark(orderID,receipt)
				if parameters["goodid"] ~= nil then
					sendMANAGERID_VALIDATE_IAP_V2(parameters["receipt"],parameters["goodid"],userid)
				elseif parameters["productid"] ~= nil then
					if orderID ~= nil then
						sendMANAGERID_VALIDATE_IAP_V2(receipt,orderID,userid) --appstore审核版本
					end
				end
			end
		...
		elseif PayResultTable["payChannel"] == profilePay.PayChannelData.HUAJIAN_DIANXIN_PAY then
			--电信短代支付
			HuaJianTelecomPay()
		elseif PayResultTable["payChannel"] == profilePay.PayChannelData.SMS_ONLINE then
			--移动短代支付
			SmsOnlinePay()
		elseif PayResultTable["payChannel"] == profilePay.PayChannelData.HUAJIAN_LIANTONG_PAY then
			--联通短代支付
			HuaJianUnicomPay()
		elseif PayResultTable["payChannel"] == profilePay.PayChannelData.EPAY then
			-- 宜支付
			if Common.platform == Common.TargetIos then
			elseif Common.platform == Common.TargetAndroid then
				--android平台
		...
		elseif PayResultTable["payChannel"] == profilePay.PayChannelData.YINBEIKEPAY_CMCC then
			-- 银贝壳移动
			if Common.platform == Common.TargetIos then
			elseif Common.platform == Common.TargetAndroid then
				--android平台
		...
		elseif PayResultTable["payChannel"] == profilePay.PayChannelData.HONGRUAN_SDK_CMCC then
			-- 红软移动
			if Common.platform == Common.TargetIos then
			elseif Common.platform == Common.TargetAndroid then
				--android平台
		...
		elseif PayResultTable["payChannel"] == profilePay.PayChannelData.YINBEIKEPAY_UNI then
			-- 银贝壳联通
			if Common.platform == Common.TargetIos then
			elseif Common.platform == Common.TargetAndroid then
				--android平台
		...
		elseif PayResultTable["payChannel"] == profilePay.PayChannelData.HONGRUAN_SDK_UNICOM then
			-- 红软联通
			if Common.platform == Common.TargetIos then
			elseif Common.platform == Common.TargetAndroid then
				--android平台
		...
		elseif PayResultTable["payChannel"] == profilePay.PayChannelData.YINBEIKEPAY_CT then
			-- 银贝壳电信
			if Common.platform == Common.TargetIos then
			elseif Common.platform == Common.TargetAndroid then
				--android平台
		...
		elseif PayResultTable["payChannel"] == profilePay.PayChannelData.HONGRUAN_SDK_CT then
			-- 红软电信
			if Common.platform == Common.TargetIos then
			elseif Common.platform == Common.TargetAndroid then
				--android平台
