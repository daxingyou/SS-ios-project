...
local DBID_V2_REICIVE_MESSAGE_table = {} --接收消息推送
...
function readisHallMessage_isRed(dataTable)
	...
	framework.emit(MAIL_UNREAD_SEND)
end
...
function setMessageReceiveAwardTable(dataTable)
	...
	framework.emit(MAIL_SYSTEM_MESSAGE_RECEIVE_AWARD_V4)
end
...
function setMessageContent(dataTable)
	...
	MessageContentUser["IsJHHVip"] = dataTable["IsJHHVip"]
...
local function read_DBID_V2_REICIVE_MESSAGE(dataTable)
	DBID_V2_REICIVE_MESSAGE_table = dataTable
	framework.emit(DBID_V2_REICIVE_MESSAGE)
end

function get_DBID_V2_REICIVE_MESSAGE_table()
	return DBID_V2_REICIVE_MESSAGE_table
end

--[[--
--检测是否还有未读系统邮件  来确定小红点的显示
--]]
function getSystemMessageIsShowRedPoint()
	if systemMessageLisTable == nil or systemMessageLisTable["SystemMessageList"] == nil or #systemMessageLisTable["SystemMessageList"] == 0 then
		return false
	end
	for i=1,#systemMessageLisTable["SystemMessageList"] do
		Common.log("systemMessageLisTable[SystemMessageList][i].MessageFlag === ==== "..systemMessageLisTable["SystemMessageList"][i].MessageFlag)
		if systemMessageLisTable["SystemMessageList"][i].MessageFlag == 0 then
			Common.log("检测是否还有未读系统邮件 按钮是否显示小红点 true")
			return true
		end
	end
	Common.log("检测是否还有未读系统邮件 按钮是否显示小红点 false")
	return false
end
...
registerMessage(MAIL_SYSTEM_MESSAGE_RECEIVE_AWARD_V4, setMessageReceiveAwardTable)
...
registerMessage(DBID_V2_REICIVE_MESSAGE, read_DBID_V2_REICIVE_MESSAGE) --接收消息推送