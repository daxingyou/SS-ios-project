...
local GIFTBAGID_GIFTBAG_LIST_SIMPLETable = {}  --首页和牌桌礼包ICON显示数据
--[[--
--获取礼包数据
]]
...
--更新礼包详情列表
local function refreshGiftDetails(mGiftBagType)
	local flag = false
	if GIFTBAGID_GIFTBAG_LIST_SIMPLETable["GiftBagData"] == nil then
		return
	end
	for key, var in pairs(GIFTBAGID_GIFTBAG_LIST_SIMPLETable["GiftBagData"]) do
		if var ~= nil and var.GiftBagType == mGiftBagType then
			--已存在
			flag = true
			break;
		end
...
function readGIFTBAGID_PUSH_DUAL_GIFTBAG(dataTable)
	...
	refreshGiftDetails(GiftData.mGiftBagType)
end
...
--返回礼包列表下标
--]]
function getGiftTableIndex(giftID)
	--GiftBagType  礼包类别ID
	for key, var in pairs(GIFTBAGID_GIFTBAG_LIST_SIMPLETable["GiftBagData"]) do
		if var ~= nil and var.GiftBagType == giftID then
			return key;
		end
	end
	return -1;
...
function readGIFTBAGID_GIFTBAG_LIST_SIMPLE(dataTable)
	GIFTBAGID_GIFTBAG_LIST_SIMPLETable = dataTable
	framework.emit(GIFTBAGID_GIFTBAG_LIST_SIMPLE)
end
function getGIFTBAGID_GIFTBAG_LIST_SIMPLETable()
	return GIFTBAGID_GIFTBAG_LIST_SIMPLETable
end
...
registerMessage(GIFTBAGID_GIFTBAG_LIST_SIMPLE, readGIFTBAGID_GIFTBAG_LIST_SIMPLE)