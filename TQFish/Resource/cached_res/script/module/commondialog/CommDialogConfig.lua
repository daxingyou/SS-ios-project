...
local getIPUrl = "http://ip.tongqutongqu.cn/";--获取IP的网址
...
local function sendPlatformMessage()
	...
	--获取GPS定位
	Common.getLocation();
	if GameConfig.getTheCurrentBaseLayer() ~= GUI_LOGIN and GameConfig.getTheCurrentBaseLayer() ~= GUI_MAHJONGTABLE then
		--不在登陆界面和不在牌桌上
		Common.getClipboard()
	end
	--微信分享下载地址
	sendMJ_SC_SHARE_INFO()

	if Common.platform == Common.TargetWindows then
	elseif Common.platform == Common.TargetIos then
		-- iOS平台
		local PayData = {
			--profilePay.PayChannelData.SMS_ONLINE, --移动短代
			--profilePay.PayChannelData.HUAJIAN_DIANXIN_PAY, --电信短代
			--profilePay.PayChannelData.RECHARGE_91 ,  -- 91
			--profilePay.PayChannelData.HAIMA_PAY ,--海马IOS
			--profilePay.PayChannelData.HUAJIAN_LIANTONG_PAY --联通短代
			profilePay.PayChannelData.ALI_PAY, -- 支付宝
			profilePay.PayChannelData.WEIXIN_PAY, -- 微信
			profilePay.PayChannelData.IAP_PAY ,   -- iap支付
		}

		if #profilePay.PayChannelData.getAliPayAllListData() == 0 or
			#profilePay.PayChannelData.getWeiXinAllListData() == 0 or
			#profilePay.PayChannelData.getIAPAllListData() == 0 then
	...
	elseif Common.platform == Common.TargetAndroid then
		--android平台
		local PayData = {
			--profilePay.PayChannelData.YINBEIKEPAY_CMCC, --银贝壳移动
			--profilePay.PayChannelData.YINBEIKEPAY_UNI, --银贝壳联通
			--profilePay.PayChannelData.YINBEIKEPAY_CT,--银贝壳电信
			--profilePay.PayChannelData.HONGRUAN_SDK_CMCC, --红软移动
			--profilePay.PayChannelData.HONGRUAN_SDK_UNICOM, --红软联通
			--profilePay.PayChannelData.HONGRUAN_SDK_CT, --红软电信
			profilePay.PayChannelData.ALI_PAY, -- 支付宝
			profilePay.PayChannelData.WEIXIN_PAY, -- 微信
			profilePay.PayChannelData.SMS_UNICOM ,-- 联通沃商店
			profilePay.PayChannelData.RECHARGE_CARD_PAY ,-- 充值卡
			profilePay.PayChannelData.NEW_UNION_PAY, --新银联支付
			profilePay.PayChannelData.SMS_ONLINE, --移动短代
			profilePay.PayChannelData.HUAJIAN_DIANXIN_PAY, --电信短代
			profilePay.PayChannelData.HUAJIAN_LIANTONG_PAY, --联通短代
			profilePay.PayChannelData.EPAY, --宜支付短代
			profilePay.PayChannelData.RECHARGE_WOJIA, --同趣联通wo+
		}

		if #profilePay.PayChannelData.getAliPayAllListData() == 0 or
			#profilePay.PayChannelData.getWeiXinAllListData() == 0 then
			sendPAYMENT_DATA_LIST(PayData) --发送支付列表请求
		end
		--友盟推送
		GamePub.UmengPushToken();
	end

	Common.getNetWorkIP(getIPUrl, uploadNetWorkIP);
end
...
function logicLogin()
	...
	if (result == 0) then
		--登录成功
		LuaUpdateConfig.logicLuaUpdateMessage();

		sendPlatformMessage();

		profileCommon.Version.setUserInitiative(false);
		--发送版本检测消息
		sendBASEID_PLAT_VERSION();

		logicIDFA();--更新AppStore版本的IDFA

		if profile.User.getSelfUserID() ~= nil then
			--微信绑定消息
			LoginConfig.sendWeChatBindMessage();

			sendMJ_SC_MGR_PLAYER_INFO(profile.User.getSelfUserID());--请求自己的基本资料
		...
		--登录成功 检测有没有未验证的iap订单
		profilePay.PaymentResult.verifyIapOrderId()
