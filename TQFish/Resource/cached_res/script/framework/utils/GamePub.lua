...
--当前是否在21点游戏中
--]]
function isInBlackJackGame()
    if GameConfig.getTheCurrentBaseLayer() == GUI_BLACKJACKTABLE then
        return true
    else
        return false
    end
...
function downloadImageForNative(picUrl, nResID, bHighPri, callBackFunction)
	...
	if picUrl == "http://f.99sai.com/assets/yuanbao/yuanbao_0001.png" then
		if Common.platform == Common.TargetIos then
			--ios平台
			local path = {};
			path["useravatorInApp"] = Common.getResourcePath("ic_ddz_tongyong_yuanbao.png")
			path["id"] = nResID
			callBackFunction(path)
			return
		elseif Common.platform == Common.TargetAndroid then
			callBackFunction(nResID.."#"..Common.getResourcePath("ic_ddz_tongyong_yuanbao.png"))
			return
		end
		return
	end
	...
	if picUrl == "http://f.99sai.com/assets/item/item26_001.png" then
		if Common.platform == Common.TargetIos then
			--ios平台
			local path = {};
			path["useravatorInApp"] = Common.getResourcePath("ic_ddz_chuangguan_fuhuoshi.png")
			path["id"] = nResID
			callBackFunction(path)
			return
		elseif Common.platform == Common.TargetAndroid then
			callBackFunction(nResID.."#"..Common.getResourcePath("ic_ddz_chuangguan_fuhuoshi.png"))
			return
		end
		return
	end
